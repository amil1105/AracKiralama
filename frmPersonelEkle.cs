    ...
    public partial class frmPersonelEkle : Form
    {
        ...
        private void btnEkle_Click(object sender, EventArgs e)
        {
            ...
            if (!bosMu)
            {
                ...
                kisikomut.Parameters.AddWithValue("@kisitipi", 'P');

                aracKiralama.ekle_sil_guncelle(kisikomut, kisiSorgu);

                string personelsorgu = "INSERT INTO personel(tc,maas,gorevi) VALUES (@tc,@maas,@gorevi)";
                NpgsqlCommand personelkomut = new NpgsqlCommand();
                personelkomut.Parameters.AddWithValue("@tc", long.Parse(txtTc.Text));
                personelkomut.Parameters.AddWithValue("@maas", long.Parse(txtMaas.Text));
                personelkomut.Parameters.AddWithValue("@gorevi", txtGorevi.Text);

                aracKiralama.ekle_sil_guncelle(personelkomut, personelsorgu);
                ...
                MessageBox.Show("Personel Ekledi!");
                foreach (Control item in Controls)
                {
                    if (item is TextBox) item.Text = "";
                }
